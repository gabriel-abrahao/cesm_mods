...
nlfolder=$modsfolder"/lists/hist_namelists/"
pefolder=$modsfolder"/hist_setup_scripts/pe_layouts/"
queue="cpu_dev"
...
echo "Setting RUN_STARTDATE in env_conf.xml to "$REFDATE
./xmlchange -file env_conf.xml -id RUN_STARTDATE -val $REFDATE
echo "Setting RUN_REFDATE in env_conf.xml to "$REFDATE
./xmlchange -file env_conf.xml -id RUN_REFDATE -val $REFDATE
...
if [ $rcp == "rcp0.0" ]; then
	cat $nlfolder/user_nl_clm >user_nl_clm
else
